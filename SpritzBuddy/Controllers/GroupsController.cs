...
namespace SpritzBuddy.Controllers
{
    ...
    public class GroupsController : Controller
    {
        ...
        // Event functionality
        [HttpGet]
        public async Task<IActionResult> CreateEvent(int groupId)
        {
            var user = await _userManager.GetUserAsync(User);
            if (user == null)
                return Unauthorized();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CreateEvent(int groupId, string title, string description, DateTime eventDate, string? location)
        {
            var user = await _userManager.GetUserAsync(User);
            if (user == null)
                return Unauthorized();
        ...
        [HttpGet]
        public async Task<IActionResult> GroupEvents(int groupId)
        {
            var user = await _userManager.GetUserAsync(User);
            if (user == null)
                return Unauthorized();
        ...
        public async Task<IActionResult> RespondToEvent(int eventId, string status)
        {
            ...
            if (success)
            {
                return Json(new { success = true, message = "Răspunsul tău a fost înregistrat!" });
            }
            else
            {
                return Json(new { success = false, message = "Nu s-a putut înregistra răspunsul." });
            }
        }

        [HttpGet]
        public async Task<IActionResult> GetGroupEventsJson(int groupId)
        {
            var user = await _userManager.GetUserAsync(User);
            if (user == null)
                return Unauthorized();
        ...
        public async Task<IActionResult> DeleteEvent(int eventId)
        {
            ...
            try
            {
                var groupEvent = await _groupService.GetEventDetailsAsync(eventId);
                var groupId = groupEvent?.GroupId;
                
                await _groupService.DeleteEventAsync(eventId, user.Id);
                TempData["Success"] = "Evenimentul a fost șters.";
                
                if (groupId.HasValue)
                    return RedirectToAction("GroupEvents", new { groupId = groupId.Value });
                return RedirectToAction("Index");
            }
            catch (UnauthorizedAccessException)
            {
